...
import type { DatePreset, SystemReportFilterValues } from '../types';

interface SystemReportFilterProps {
  filters: SystemReportFilterValues;
  onChange: (filters: SystemReportFilterValues) => void;
  onApply: () => void;
  onReset: () => void;
...
export const SystemReportFilter: React.FC<SystemReportFilterProps> = ({
  filters,
  onChange,
  onApply,
  onReset,
  exportData,
  exportFilename = 'Airport_System_Report',
  ...
  const handleExport = (type: 'csv' | 'excel' | 'pdf' | 'print') => {
    ...
    if (type === 'csv') {
      ...
      alert('Excel export initiated for current filtered system dataset.');
    }
  };

  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-xs space-y-3.5">
      {/* Top Bar: Quick Presets & Export */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {[
            ...
            { id: '7days' as DatePreset, label: 'Last 7 Days' },
            { id: '30days' as DatePreset, label: 'Last 30 Days' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        {/* Call Status */}
        <div>
          <label className="block text-[11px] font-bold text-slate-600 mb-1">Call Status</label>
          <select
            value={filters.callStatus}
            onChange={(e) => onChange({ ...filters, callStatus: e.target.value })}
            className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
          >
            {systemCallStatuses.map((st) => (
        ...
        {/* Extension Range */}
        <div>
          <label className="block text-[11px] font-bold text-slate-600 mb-1">Extension Range</label>
          <input
            type="text"
            value={filters.extensionRange}
            onChange={(e) => onChange({ ...filters, extensionRange: e.target.value })}
            placeholder="e.g. EXT-1000 - EXT-2999"
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5 pt-1 border-t border-slate-100">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input
            ...
            placeholder="Search by Extension, Department, Destination or CDR ID..."
