...
import type { CMRQuickFilterKey } from '../types';

interface CMRQuickFiltersProps {
  searchQuery: string;
  activeQuickFilter: CMRQuickFilterKey;
  isAdvancedOpen: boolean;
  onSearchChange: (query: string) => void;
  onQuickFilterChange: (filter: CMRQuickFilterKey) => void;
  onToggleAdvanced: () => void;
  onClearSearch: () => void;
}

export const CMRQuickFilters: React.FC<CMRQuickFiltersProps> = ({
  searchQuery,
  activeQuickFilter,
  isAdvancedOpen,
  onSearchChange,
  ...
  const quickFilters: { id: CMRQuickFilterKey; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
    { id: 'all', label: 'All CMR', icon: Layers },
    { id: 'excellent', label: 'Excellent (≥90)', icon: Award },
    { id: 'good', label: 'Good (75-89)', icon: CheckCircle2 },
    { id: 'fair', label: 'Fair (60-74)', icon: ShieldCheck },
    { id: 'poor', label: 'Poor (<60)', icon: ShieldAlert },
    { id: 'quality-issues', label: 'Quality Issues', icon: AlertTriangle },
    { id: 'high-jitter', label: 'High Jitter (>20ms)', icon: Radio },
    { id: 'high-packet-loss', label: 'High Loss (>1%)', icon: Network },
    { id: 'high-latency', label: 'High Latency (>100ms)', icon: Clock },
  ];
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search CMR ID, CDR ID, extension, device, or user..."
        ...
        >
          ...
          <span>Advanced QoS Filters</span>
        </button>
      </div>
